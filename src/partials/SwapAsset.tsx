...
const AssetPickerTrigger = ({ asset, disabled }: AssetPickerTriggerProps) => (
  <Dialog.Trigger
    type="button"
    disabled={disabled}
    className={cn(
...
export default function SwapAsset({ defaultAsset = "" }: SwapAssetProps) {
  ...
  const maxAmount = useMemo(
    () =>
      calculateAssetMaxAmount(
        selectedSourceAsset as unknown as HorizonBalance,
  ...
  const handleFormSubmit = async (data: SwapFormValues) => {
  ...
  /** Handle Picker */
  const handleSourceAssetPicker = (asset: EnrichedBalance) => {
  ...
  /** Handle Picker */
  const handleReceivedAssetPicker = (asset: EnrichedBalance) => {
  ...
  useDebounce(
    () => {
      if (calculatorRef.current.calculatedSource) {
        ...
        return;
      } else if (new Decimal(sourceAmount).greaterThan(0)) {
  ...
  useDebounce(
    () => {
      if (calculatorRef.current.calculatedReceived) {
        ...
        return;
      } else if (new Decimal(receivedAmount).greaterThan(0)) {
