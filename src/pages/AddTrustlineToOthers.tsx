...
export default function AddTrustlineToOthers() {
  const { account, accountQuery, asset, publicKey } =
    ...
    await execute(
      async (source) => {
        ...
        if (!exists) {
          ...
          const canAddTrustline = new Decimal(
            XLMAsset ? XLMAsset["balance"] : 0,
          ).gte(new Decimal(requiredXLM));
  ...
  return (
    <div className="flex flex-col gap-4 grow min-h-0 min-w-0">
      ...
      {/* Asset */}
      <div className="p-4 flex flex-col gap-2 bg-neutral-900 rounded-xl">
        {/* Icon and Name */}
        ...
        <div className="flex flex-col">
          ...
          <p
            ...
            onClick={() => copyToClipboard(assetCode)}
          >
            <IoCopyOutline className="size-4 inline-flex" />{" "}
            <span className="font-bold">Asset Code:</span> {assetCode}
          </p>

          {/* Asset Issuer */}
          <p
            className="truncate cursor-pointer"
            onClick={() => copyToClipboard(assetIssuer)}
          >
            <IoCopyOutline className="size-4 inline-flex" />{" "}
            <span className="font-bold">Asset Issuer:</span>{" "}
            <span className="truncate">{assetIssuer}</span>
          </p>
